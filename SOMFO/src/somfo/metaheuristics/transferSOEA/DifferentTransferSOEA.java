package somfo.metaheuristics.transferSOEA;
...
public class DifferentTransferSOEA extends Algorithm{
	public DifferentTransferSOEA(ProblemSet problem) {
		super(problem);
	...
	public Population execute() throws JMException, ClassNotFoundException {
		...
		for(int i = 0; i< problemSet_.size();i++){
			fileSubscription.printToFile("result/FUN/"  + "DifferentTransfer" + "/"+ directoryname   + "/" + name + "/"+ "differentInd" + problemSet_.get(i).getName() + (time -1)+ ".dat", number[i], populationSize_);

			fileSubscription.printToFile("result/FUN/"  + "DifferentTransfer" + "/"+ directoryname   + "/" + name + "/" + problemSet_.get(i).getName() + (time -1)+ ".dat", best[i], populationSize_);
		}
