...
public class TransferSOEA extends Algorithm{

	public TransferSOEA(ProblemSet problem) {
		super(problem);
...
//		int maxEvaluations = ((Integer) this.getInputParameter("maxEvaluations")).intValue();
		...
		double[][] bestinddata  = new double[problemSet_.size()][maxGenerations];
		...
		for(int i = 0;i<problemSet_.size();i++){
			...
			initPopulation(i);
			gen[i] = 0;
			bestinddata[i][0] = population_[i].getMINFactorialCost(0);
		}
		...
		//		System.out.println("Task" + (t+1) +  "	"+evaluation[t]);
				...
				for(int e = 0; e < everygeneration_;e++){
					...
					bestinddata[t][++gen[t]] = population_[t].getMINFactorialCost(0);
