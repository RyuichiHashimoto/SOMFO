...
public class IslandModelSOEA extends Algorithm{
	public IslandModelSOEA(ProblemSet problem) {
		super(problem);
...
//			fileSubscription.printToFile(directoryname   +  "/Task"+(i+1)+ "/differentInd/" + problemSet_.get(i).getName() + (time -1)+ ".dat", number[i], populationSize_);
			...
			//fileSubscription.printToFile(directoryname   +  "/Task"+(i+1)+"/BestFUN/" + problemSet_.get(i).getName() + (time)+ "_.dat", best[i], 1);
