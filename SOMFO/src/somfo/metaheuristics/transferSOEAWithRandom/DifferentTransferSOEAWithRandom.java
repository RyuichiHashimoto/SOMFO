...
public class DifferentTransferSOEAWithRandom extends Algorithm{
	public DifferentTransferSOEAWithRandom(ProblemSet problem) {
		super(problem);
	...
	public Population execute() throws JMException, ClassNotFoundException {
		...
		for(int i = 0; i< problemSet_.size();i++){
			fileSubscription.printToFile("result/FUN/"  + "DifferentTransferSOEAWithRandom" + "/"+ directoryname   + "/" + name + "/"+ "differentInd"  + problemSet_.get(i).getName() + (time -1)+ ".dat", number[i], populationSize_);
			fileSubscription.printToFile("result/FUN/"  + "DifferentTransferSOEAWithRandom" + "/"+ directoryname   + "/" + name + "/" + problemSet_.get(i).getName() + (time -1)+ ".dat", best[i], populationSize_);
	...
	public void transfer_pop(int key ) throws ClassNotFoundException{
		...
		for(int i = 1;d.size()<(populationSize_ - amoutOftransfer_ )|| (i < population_[key].size());i++){
			...
			for(int k = 0;k<d.size();k++){
				...
				//if two solution is different dd have false
				if(Solution.IsEqual(a, population_[key].get(i))){
					dd = false;
					break;
				}
			}
			// true means Thare are no solution equal Solution in d;
			if(dd){
		...
		for(int i = 0;i<populationSize_ - amoutOftransfer_ - d.size();i++){
			...
			int keke = Random.nextIntIE(0, population_[kuruma].size());
			d.add(new Solution(population_[kuruma].get(keke)));
		}
			...
			for(int j = 0; j < problemSet_.size();j++ ){
				...
				for(int m = 0;m< amoutOftransfer_ ;m++){
					Solution ded = new Solution(problemSet_);
					double sum = problemSet_.get(key).evaluate(ded);
					ded.setFactorialCost(0,sum);
					evaluations_++;
					d.add(ded);
				}
			}
