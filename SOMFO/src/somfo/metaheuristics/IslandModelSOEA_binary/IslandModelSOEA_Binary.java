package somfo.metaheuristics.IslandModelSOEA_binary;
...
public class IslandModelSOEA_Binary extends Algorithm{

	public IslandModelSOEA_Binary(ProblemSet problem) {
		super(problem);
...
//		Permutation.randomPermutation(randomPerm);
		...
		for(int t = 0; t< problemSet_.size();t++){
			...
			//	double sum = problemSet_.get(key).evaluate(empty);
				empty.setFactorialCost(0,1.0E100);
				//evaluate++;
				other.add(new Solution(empty));
			}
		}
		...
		for(int pop = 0; pop < amoutOftransfer_;pop++){
			...
			int first = 0;
			do {
				first = Random.nextIntIE(other.size());
			} while(list.contains(first));

			int second = 0;
			do {
				second = Random.nextIntIE(other.size());
			} while (first == second || list.contains(second));

			double firstFun = problemSet_.get(key).evaluate(other.get(first));
			double secondFun = problemSet_.get(key).evaluate(other.get(second));
			if(firstFun < secondFun){
				Solution test = other.get(first);
				d.add(test);
			} else if (firstFun < secondFun){
				Solution test = other.get(second);
				d.add(test);
			} else {
