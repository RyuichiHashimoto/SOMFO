package somfo.metaheuristics.reverse_transferSOEA;
...
public class Reverse_TransferSOEA extends Algorithm{

	public Reverse_TransferSOEA(ProblemSet problem) {
		super(problem);
...
//		int maxEvaluations = ((Integer) this.getInputParameter("maxEvaluations")).intValue();
		...
		double[][] bestpergen = new double[problemSet_.size()][maxGenerations];
		...
		for(int i = 0;i<problemSet_.size();i++){
			...
			initPopulation(i);
			gen[i] = 0;
			bestpergen[i][0] = population_[i].getMINFactorialCost();
		}
		...
		//		System.out.println("Task" + (t+1) +  "	"+evaluation[t]);
				...
				for(int e = 0; e < everygeneration_;e++){
					...
					bestpergen[t][++gen[t]] = population_[t].getMINFactorialCost();
				}
			}
			counter = 0;
...
//			fileSubscription.printToFile(directoryname   +  "/Task"+(i+1)+ "/differentInd/" + problemSet_.get(i).getName() + (time -1)+ ".dat", number[i], populationSize_);
			fileSubscription.printToFile(directoryname   +  "/Task"+(i+1)+"/BestFUN/" + problemSet_.get(i).getName() + (time)+ ".dat", bestpergen[i], 1);


			population_[i].printVariablesToFile(directoryname+ "/Task"+ (i+1)+"/BestVAR/"+ problemSet_.get(i).getName() + (time)+ ".dat");
			//fileSubscription.printToFile(directoryname   +  "/Task"+(i+1)+"/Different/" + problemSet_.get(i).getName() + (time)+ "data.dat",  best[i], 1);
