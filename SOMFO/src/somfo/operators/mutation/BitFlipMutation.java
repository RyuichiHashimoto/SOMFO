...
public class BitFlipMutation extends Mutation {
	...
	/*	try {
			if ((solution.getType().getClass() == BinarySolutionType.class)
					|| (solution.getType().getClass() == BinaryRealSolutionType.class) ||(solution.getType().getClass() == BinaryRealSolutionType.class) ) {
			...
			else if ((solution.getType().getClass() == BinaryIntSolutionType.class)){
				for (int i = 0; i < solution.getDecisionVariables().length; i++)
					if (Random.nextDoubleIE()< probability) {
						if(solution.getDecisionVariables()[i].getValue() > 0.9){
							solution.getDecisionVariables()[i].setValue(0.0);
						} else {
							solution.getDecisionVariables()[i].setValue(1);
						}
					} // if
			}
				...
				for (int i = 0; i < solution.getDecisionVariables().length; i++)
					if (Random.nextDoubleIE()< probability) {
						int value = Random.nextIntIE((int) solution.getDecisionVariables()[i].getUpperBound() - (int) solution.getDecisionVariables()[i].getLowerBound() + 1) + (int) solution.getDecisionVariables()[i].getLowerBound();
