...
import somfo.metaheuristics.IslandModelSOEA_with_rate.IslandModelSOEA_with_rate;
...
public class IslandMain_with_rate extends AlgorithmMain{

	public IslandMain_with_rate(Setting test) {
		super(test);
	}
	...
	public void setParameter() throws NameNotFoundException, ClassNotFoundException, JMException {
		...
		Operator Mutation;
		Operator Crossover;
		DirectoryName = "result/Island_with_rate/" + taskname;
		algorithm = new Algorithm[1];
		algorithm[0] = new IslandModelSOEA_with_rate(problem_);
...
//		algorithm[0].setInputParameter("tasknumber", "Task" + String.valueOf(i+1) );
		DirectoryName = "result/Island_with_rate" +"/every" + setting_.getAsStr("every") +"amount" + setting_.getAsStr("amount")+"/"+ taskname;
