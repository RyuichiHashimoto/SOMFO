...
import somfo.metaheuristics.transferSOEA.TransferSOEA;
...
public class TransferMain extends AlgorithmMain{

	public TransferMain(Setting test) {
		super(test);
	}
	...
	public void setParameter() throws NameNotFoundException, ClassNotFoundException, JMException {
		...
		Operator Mutation;
		Operator Crossover;
		DirectoryName = "result/Transfer/" + taskname;
		algorithm = new Algorithm[1];
		algorithm[0] = new TransferSOEA(problem_);
...
//		algorithm[0].setInputParameter("tasknumber", "Task" + String.valueOf(i+1) );
		DirectoryName = "result/Transfer" +"/every" + setting_.getAsStr("every") +"amount" + setting_.getAsStr("amount")+"/"+ taskname;
