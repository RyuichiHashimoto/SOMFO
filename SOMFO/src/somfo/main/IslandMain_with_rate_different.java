...
import somfo.metaheuristics.IslandModelSOEA_with_rate_innerSwap.IslandModelSOEA_with_rate_Different;
...
public class IslandMain_with_rate_different extends AlgorithmMain{

	public IslandMain_with_rate_different(Setting test) {
		super(test);
	}
	...
	public void setParameter() throws NameNotFoundException, ClassNotFoundException, JMException {
		...
		Operator Mutation;
		Operator Crossover;
		Operator InnerCrossover;
		DirectoryName = "result/Island_with_rate_Different/" + taskname;
		algorithm = new Algorithm[1];
		algorithm[0] = new IslandModelSOEA_with_rate_Different(problem_);
		...
		InnerCrossover = CrossoverFactory.getCrossoverOperator((String)setting_.getAsStr("differentTaskCrossoverName"), parameters);

		algorithm[0].addOperator("crossover", Crossover);

		algorithm[0].addOperator("differentTaskCrossover", InnerCrossover);
...
//		algorithm[0].setInputParameter("tasknumber", "Task" + String.valueOf(i+1) );
		DirectoryName = "result/Island_with_rate_Different" +"/every" + setting_.getAsStr("every") +"amount" + setting_.getAsStr("amount")+"/"+ taskname;
