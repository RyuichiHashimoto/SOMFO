...
import somfo.metaheuristics.ReEvaluate_Transfer.ReEvaluate_TransferSOEA;
...
public class ReEvaluate_TransferMain extends AlgorithmMain{

	public ReEvaluate_TransferMain(Setting test) {
		super(test);
	}
	...
	public void setParameter() throws NameNotFoundException, ClassNotFoundException, JMException {
		...
		Operator Mutation;
		Operator Crossover;
		DirectoryName = "result/Re_Transfer/" + taskname;
		algorithm = new Algorithm[1];
		algorithm[0] = new ReEvaluate_TransferSOEA(problem_);
...
//		algorithm[0].setInputParameter("tasknumber", "Task" + String.valueOf(i+1) );
		DirectoryName = "result/Re_Transfer" +"/every" + setting_.getAsStr("every") +"amount" + setting_.getAsStr("amount")+"/"+ taskname;
