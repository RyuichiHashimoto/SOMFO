...
import somfo.metaheuristics.transferSOEAWithRandom.TransferSOEAWithRandom;
...
public class RandomMain extends AlgorithmMain{

	public RandomMain(Setting test) {
		super(test);
	}
	...
	public void setParameter() throws NameNotFoundException, ClassNotFoundException, JMException {
		...
		Operator Mutation;
		Operator Crossover;
		DirectoryName = "result/Random/" + taskname;
		algorithm = new Algorithm[1];
		algorithm[0] = new TransferSOEAWithRandom(problem_);
...
//		algorithm[0].setInputParameter("tasknumber", "Task" + String.valueOf(i+1) );
		DirectoryName = "result/Random" +"/every" + setting_.getAsStr("every") +"amount" + setting_.getAsStr("amount")+"/"+ taskname;
