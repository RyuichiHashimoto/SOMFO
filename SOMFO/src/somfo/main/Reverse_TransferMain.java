...
import somfo.metaheuristics.reverse_transferSOEA.Reverse_TransferSOEA;
...
public class Reverse_TransferMain extends AlgorithmMain{

	public Reverse_TransferMain(Setting test) {
		super(test);
	}
	...
	public void setParameter() throws NameNotFoundException, ClassNotFoundException, JMException {
		...
		Operator Mutation;
		Operator Crossover;
		DirectoryName = "result/Reverse_Transfer/" + taskname;
		algorithm = new Algorithm[1];
		algorithm[0] = new Reverse_TransferSOEA(problem_);
...
//		algorithm[0].setInputParameter("tasknumber", "Task" + String.valueOf(i+1) );
		DirectoryName = "result/Reverse_Transfer" +"/every" + setting_.getAsStr("every") +"amount" + setting_.getAsStr("amount")+"/"+ taskname;
