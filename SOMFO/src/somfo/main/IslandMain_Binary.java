...
import somfo.metaheuristics.IslandModelSOEA_binary.IslandModelSOEA_Binary;
...
public class IslandMain_Binary extends AlgorithmMain{

	public IslandMain_Binary(Setting test) {
		super(test);
	}
	...
	public void setParameter() throws NameNotFoundException, ClassNotFoundException, JMException {
		...
		Operator Mutation;
		Operator Crossover;
		DirectoryName = "result/Island_binary/" + taskname;
		algorithm = new Algorithm[1];
		algorithm[0] = new IslandModelSOEA_Binary(problem_);
...
//		algorithm[0].setInputParameter("tasknumber", "Task" + String.valueOf(i+1) );
		DirectoryName = "result/Island_binary" +"/every" + setting_.getAsStr("every") +"amount" + setting_.getAsStr("amount")+"/"+ taskname;
