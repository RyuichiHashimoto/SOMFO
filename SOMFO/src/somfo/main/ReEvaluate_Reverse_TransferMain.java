...
import somfo.metaheuristics.ReEvaluate_reverse_Transfer.ReEvaluate_Reverse_TransferSOEA;
...
public class ReEvaluate_Reverse_TransferMain extends AlgorithmMain{

	public ReEvaluate_Reverse_TransferMain(Setting test) {
		super(test);
	}
	...
	public void setParameter() throws NameNotFoundException, ClassNotFoundException, JMException {
		...
		Operator Mutation;
		Operator Crossover;
		DirectoryName = "result/ReEvalute_Reverse_Transfer/" + taskname;
		algorithm = new Algorithm[1];
		algorithm[0] = new ReEvaluate_Reverse_TransferSOEA(problem_);
...
//		algorithm[0].setInputParameter("tasknumber", "Task" + String.valueOf(i+1) );
		DirectoryName = "result/ReEvalute_Reverse_Transfer" +"/every" + setting_.getAsStr("every") +"amount" + setting_.getAsStr("amount")+"/"+ taskname;
